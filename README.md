# ChannelInsight — YouTube 채널·영상 콘텐츠 분석

YouTube 채널 또는 개별 영상 URL을 입력하면 **Google Gemini** 기반으로 한국어 전략 리포트를 생성하는 웹 앱입니다. 선택적으로 **YouTube Data API**로 수집한 실시간 메타데이터(조회수, 구독자, 최근 영상 등)를 분석에 반영할 수 있습니다.
...
이 앱은 **구독자 성장**, **조회·유지 개선**, **수익화**(광고·쇼츠·제휴 등)를 동시에 다루는 전략 리포트를 목표로 합니다. 알고리즘 관점에서는 단기 조회수보다 **장기 시청자 만족도**(클릭 이후 시청 경험·재방문)를 우선하는 해석을 따릅니다. 세부 원칙·백로그는 루트의 `Agent.md`와 동일합니다.
...
| **내보내기** | 분석 결과를 `.md` 파일로 다운로드하거나, 웹 페이지 형태로 새 창에서 열어볼 수 있습니다. |
...
애플리케이션 소스의 일부 파일(예: `src/App.tsx`)에는 **Apache License 2.0**(`SPDX-License-Identifier: Apache-2.0`) 표기가 포함되어 있습니다. 저장소 루트에 별도의 `LICENSE` 파일이 없다면, 배포·재사용 시 해당 SPDX 주석이 있는 파일의 라이선스 조건을 따르고, 필요하면 프로젝트 전체에 맞는 `LICENSE` 파일을 추가하는 것을 권장합니다.

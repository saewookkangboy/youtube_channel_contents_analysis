/**
 * 수집(collect) 단계: YouTube Data API 팩트 fetch와 분석 준비(로컬에서 켠 경우 형식 보강 접미사 동적 로드)를
 * `Promise.all`로 병렬 실행해, 검증 단계의 OpenAI·Gemini 병렬과 같은 **레인 병렬** 패턴을 맞춘다.
 * 이후 정제·리포트 단계에서는 팩트가 있으면 Gemini `text-embedding-004` 임베딩과 (프리패치 시 생략된) 형식 보강 접미사 로드를 추가로 병합한다.
 */

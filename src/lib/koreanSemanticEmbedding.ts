 ...
 */
export async function buildVideoKoreanSemanticGroundingBlock(data: YouTubeVideoData): Promise<string> {
  ...
  try {
    const vectors = await embedTexts([title, ...chunks]);
...
export async function buildChannelKoreanSemanticGroundingBlock(
  ...
  try {
    const vectors = await embedTexts([name, ...titles]);
